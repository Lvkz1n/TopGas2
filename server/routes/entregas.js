...
import { generateCSV, setCSVHeaders, asyncHandler, isValidId, calcularTempoTotal } from "../utils.js";
...
r.get("/test-data", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const { rows } = await query(`
...
r.get("/csv", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const { rows: entregas } = await query(`SELECT * FROM topgas_entregas ORDER BY id DESC`);
  ...
  const entregasComTempo = entregas.map(entrega => {
    ...
    return {
      ...entrega,
      tempo_total: calcularTempoTotalEntrega(entrega.data_e_hora_inicio_pedido, dataFim)
    };
  });
  
  const csv = generateCSV(entregasComTempo, ENTREGAS_CSV_CONFIG.headers, ENTREGAS_CSV_CONFIG.fieldMapping);
...
r.post("/:id/confirmar", requireAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
...
r.post("/:id/cancelar", requireAuth, asyncHandler(async (req, res) => {
  const { id } = req.params;
